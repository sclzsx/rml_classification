...
def showCAM(cam_thr, reserve_erase):
   ...
   # Dataset
    if cfgs.dataset_name == "RML2016.10a":
    ...
    for images, labels in dataloader_valid:
        ...
        for name in range(10):
            labels[0] = name
            print("change:", labels[0])
        '''
        # Grad-CAM++
        cams, scores, pred_labels = compute_gradcampp(images, labels, model, gt_known = True)     # (bz, 128, 2)
        # SigCAM
        # cams_target, cams, scores, pred_labels = compute_Sigcam(images, labels, model, gt_known = True)     # (bz, 128, 2)
        
        for image, cam, label, pred_label in zip(images, cams, labels, pred_labels):
            if cam.max() == 0:
                # showCamSignal(t2n(image), cam, label, idx)
                CAM_ERROR.append((t2n(label), t2n(pred_label)))
                print("CAM is 0, break", label, pred_label)
                continue
            idx += 1
            # showImgSignal(t2n(image), label)              # 绘制处理后的二维信号
            # showOriSignal(t2n(image), label, idx)         # 绘制原始信号图
            # showCamSignal(t2n(image), cam_target, label, idx)    # 绘制CAM叠加图
...
def showGBP():
   ...
   # Dataset
    ...
    for images, labels in dataloader_valid:
        ...
        showCamSignal(guided_grads[0], cams[0], labels[0], 1)
        ''' FOR IMAGE
        # 保存伪彩梯度图
