...
class DefaultConfigs_RML(object):
    ''' 默认参数配置 '''
    # Dataset
    dataset_name = "RML2016.10a"                # RML2016.04c(6db,50): 89.0689
    num_classes = 11                            # 分类类别数
    signal_len = "128,2"
    train_path = 'C:/Users/sunxin/Desktop/signal/Signal_master-master/dataset/RML2016_10a/6dB_SNR/6dB-SNR_500-train.npy'  # 原始训练数据目录
    test_path = 'C:/Users/sunxin/Desktop/signal/Signal_master-master/dataset/RML2016_10a/6dB_SNR/6dB-SNR_500-test.npy'    # 原始测试数据目录
    process_IQ = True                           # 是否在载入数据时对IQ两路进行预处理

    batch_size = 110                            # DataLoader中batch大小，550/110=5 Iter
    num_workers = 0                             # DataLoader中的多线程数量

    # model
    model = "VGG16"                   # 指定模型，目前就一个
    resume = False                              # 是否加载训练好的模型
    checkpoint_name = 'VGG16_RML2016_10a.pth' # 训练完成的模型名
